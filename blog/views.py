...
def likes_dislikes(request, blog_id, action):
        ...
        if user.is_authenticated:
            if action == 'like':
                if user in blog.liked_by.all():
                    blog.liked_by.remove(user) # User has already liked, so remove the user
                else:
                ...
                if user in blog.disliked_by.all():
                    blog.disliked_by.remove(user) # User has already liked, so remove the user
                else:
    ...
    return JsonResponse({'likes': blog.liked_by.count(), 'dislikes': blog.disliked_by.count()})
